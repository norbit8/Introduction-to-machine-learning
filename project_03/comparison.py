...
if "__main__" == __name__:
    ...
    for m in nums:
        ...
        df2 = DataFrame({'x1_orange': x1_orange, 'x2_orange': x2_orange, 'tags': -1*np.ones(len(x2_orange))})  # Orange dots
        df3 = DataFrame({'x': line, "hp": hp, "name": ['True hypothesis hyperplane']*len(hp)})  # True hypothesis HyperPlane
        ...
        geom_point(aes(x='x1_orange', y='x2_orange', fill='factor(tags)'), stroke=0, data=df2, color="orange", size=2) + \
        ...
        labs(x="Feature 1 ($x_1$)", y="Feature 2 ($x_2$)", fill="Labels", title=f"Question 9: Testing SVM algorithm VS"
                                                                                f" Perceptron (number of samples:{m})"))
        print(p)
        ggsave(p, f"plot{m}.png" , width=4, height=4)

        # geom_point(aes(x='x1', y='x2'), size=3, color="orange") +\
        # scale_fill_manual(values=["orange", "blue"]) + \
        # geom_point(aes(x='x', y='hp'), color="black") +
        # geom_smooth(aes(x='x', y='hp'), method="lm", color="black") + \
        # labs(x="x", y="y", fill="Label", title="Question 9: "))
        # print(p)
